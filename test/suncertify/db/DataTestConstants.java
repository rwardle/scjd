...
public class DataTestConstants {
    ...
    static {
        ...
        for (int i = 0; i < EXPECTED_FIELD_COUNT; i++) {
            EXPECTED_FIELD_DESCRIPTIONS[i] = new FieldDescription(FIELD_NAMES[i], FIELD_LENGTHS[i],
                    recordOffset);
            recordOffset += EXPECTED_FIELD_DESCRIPTIONS[i].getLength();
        }
    }

    public static final String[] RECORD_VALUES = { "Buonarotti & Company", "Smallville",
            "Air Conditioning, Painting, Painting", "10", "$40.00", "1245678" };
    public static final String[] RECORD_VALUES_SPACE_PADDED, RECORD_VALUES_NULL_PADDED;
