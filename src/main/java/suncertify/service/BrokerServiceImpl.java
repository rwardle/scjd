...
public final class BrokerServiceImpl implements BrokerService {
    ...
    private static final Logger LOGGER = Logger.getLogger(BrokerServiceImpl.class.getName());
     ...
     * {@inheritDoc}
     */
    public List<Contractor> search(SearchCriteria searchCriteria) throws IOException {
        ...
        /*
         * Database find does "starts-with" matches, so read each record from the database and only
         * include the ones that still exist and are exact matches.
         */
        ...
        for (int recNo : recNos) {
            try {
                ...
                LOGGER.warning("Record " + recNo + " not found, excluding from search results.");
            }
        }
     ...
     * {@inheritDoc}
     */
    public void book(String customerId, Contractor contractor) throws IOException,
        ...
        if (customerId.length() != CUSTOMER_ID_LENGTH) {
            throw new IllegalArgumentException("customerId must be of length " + CUSTOMER_ID_LENGTH);
        }
        if (!containsOnlyDigits(customerId)) {
            throw new IllegalArgumentException("customerId must only contain digits");
        }
        ...
        /*
         * Contractor should not have any null fields - empty record values are represented by an
         * empty string.
         */
        if (hasNullField(contractor)) {
            throw new IllegalArgumentException("contractor cannot have a null field");
        }
    ...
    private boolean hasNullField(Contractor contractor) {
        return contractor.getName() == null || contractor.getLocation() == null
                || contractor.getSpecialties() == null || contractor.getSize() == null
                || contractor.getRate() == null || contractor.getOwner() == null;
    }

    private void lockRecord(int recNo) throws ContractorDeletedException, IOException {
        try {
            database.lock(recNo);
        } catch (RecordNotFoundException e) {
            throw new ContractorDeletedException(e);
            ...
            IOException ioException = new IOException(
                    "Thread was interrupted while waiting for the lock on recNo: " + recNo);
    ...
    private void validateRecord(int recNo, Contractor contractor)
            throws ContractorModifiedException, RecordNotFoundException, IOException {
    ...
    private boolean isModified(Contractor contractor, String[] data) {
        ...
        /*
         * If the only field that does not match is the owner and the owner is an empty string then
         * that implies that the record has been unbooked by another user. Allow the booking to
         * continue in this case (i.e. return false from this method).
         */
        boolean ownerFieldEmpty = data[ServiceConstants.OWNER_FIELD_INDEX].equals("");
        boolean ownerFieldModified = !data[ServiceConstants.OWNER_FIELD_INDEX].equals(contractor
                .getOwner());
        return modified || !ownerFieldEmpty && ownerFieldModified;
    }

    private void updateRecord(int recNo, String customerId) throws RecordNotFoundException,
            IOException {
        /*
         * Only need to update the owner field, all other fields are null to signify that they
