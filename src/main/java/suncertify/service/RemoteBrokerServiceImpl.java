...
public final class RemoteBrokerServiceImpl extends UnicastRemoteObject implements
    ...
    private static final Logger LOGGER = Logger.getLogger(RemoteBrokerServiceImpl.class.getName());
    ...
    public List<Contractor> search(SearchCriteria searchCriteria) throws IOException {
        try {
            ...
            LOGGER.info("Found " + contractors.size() + " contractors exactly matching criteria: "
                    + searchCriteria);
            return contractors;
        } catch (IOException e) {
            LOGGER.log(Level.SEVERE, "IO error while searching for contractors with criteria: "
                    + searchCriteria, e);
            throw e;
        }
    }
    ...
    public void book(String customerId, Contractor contractor) throws IOException,
        ...
        try {
            ...
            LOGGER.info("Customer with ID=" + customerId + " has booked contractor: " + contractor);
        } catch (IOException e) {
            LOGGER.log(Level.SEVERE, "IO error while booking contractor", e);
            throw e;
        } catch (ContractorDeletedException e) {
            LOGGER.log(Level.SEVERE, "Contractor to be booked has been deleted, recordNo: "
                    + contractor.getRecordNumber(), e);
            throw e;
        } catch (ContractorModifiedException e) {
            LOGGER.log(Level.SEVERE, "Contractor to be booked has been modified, recordNo: "
                    + contractor.getRecordNumber(), e);
            throw e;
        }
    }
